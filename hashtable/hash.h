...
public:
    ...
    int hashValue(char key) { return key % size; }
    ...
    bool keysEqual(T k1, T k2) {
        if (std::is_same<T, char*>::value) { // runtime type check
    ...
    bool containsKey(T key) {
        ...
        Node<T,T1>* temp = arr[index].getHead();
        while (temp != NULL) {
            if (keysEqual(temp->key, key)) return true;
            temp = temp->next;
        }
        return false;
    }
    ...
    LinkedList<T,T1>* buckets() { return arr; }
